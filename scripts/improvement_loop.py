...
class ExecutorAgent:
    ...
    FIX_REGISTRY: list[dict] = [
        {"id": "add_get_email_full_body", "description": "Add get_email_full_body tool that returns untruncated email bodies for specific message_ids", "target_file": "src/agent/agent_serving.py", "priority": 1},
        {"id": "fix_search_emails_thread_id", "description": "Fix search_emails to include thread_id and to_list in returned JSON", "target_file": "src/agent/agent_serving.py", "priority": 2},
        {"id": "increase_snippet_lengths", "description": "Increase snippet_length from 500->1000 and body_preview from 300->800 in evidence tools", "target_file": "src/agent/agent_serving.py", "priority": 3},
        {"id": "evidence_chaining_prompt", "description": "Add evidence-chaining guidance to ENRON_SYSTEM_PROMPT", "target_file": "src/agent/agent_serving.py", "priority": 4},
        {"id": "register_full_body_tool", "description": "Register get_email_full_body in LOCAL_TOOLS and system prompt tool list", "target_file": "src/agent/agent_serving.py", "priority": 5},
    ]
