...
hardening patches directly to ``src/agent/_agent_core.py`` (implementation behind the serving shim).
...
AGENT_SERVING_PATH = Path("src/agent/_agent_core.py")
ORCHESTRATOR_PATH = Path("src/_internal/agent/factual_parallel_orchestrator.py")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Repository root used to locate src/agent/_agent_core.py for apply mode.",
    )
