"""
GraphRAG agent implementation source (loaded by ``agent_serving.py`` via importlib).

Do not ``import src.agent._agent_core`` — use ``import src.agent.agent_serving`` so there
is a single module dict for monkeypatches and MLflow entrypoints.
"""

if __name__ == "src.agent._agent_core" and __package__:
    raise ImportError(
        "GraphRAG agent implementation is private; use `import src.agent.agent_serving`."
    )
...
def resolve_entity(name: str) -> ResolvedEntity:
    ...
    if not email_addresses and len(cparts) >= 2:
        first, last = cparts[0], cparts[-1]
        stem = last[:-2] if len(last) > 4 else last
        try:
            rows = _backend.execute_sql(
                f"SELECT DISTINCT email_address, name_normalized, display_name FROM {ENRON_PARTICIPANTS_TABLE}"
            ...
            for r in (rows or []):
                ...
                if (
                    addr
                    and addr not in email_pats
                    and _participant_row_matches_name(r, first=first, last=last)
                ):
                    fallback_addresses.append(addr)
                    email_addresses.append(addr)
            if fallback_addresses:
                # Prepend validated fallback addresses so exact identities win before wildcards.
                email_pats = fallback_addresses + email_pats
        except Exception:
            pass
...
def _person_name_tokens(value: str) -> list[str]:
    """Split a person-like string into lowercase alphanumeric tokens."""
    return [tok for tok in re.split(r"[^a-z0-9]+", (value or "").lower()) if tok]


def _email_candidate_matches_name(email: str, first: str, last: str) -> bool:
    """Return True when an email address plausibly belongs to first/last."""
    raw_local = email.split("@")[0].lower() if "@" in email else email.lower()
    raw_tokens = [tok for tok in re.split(r"[._\-]+", raw_local) if tok]
    normalized_local = _email_local_part(email)
    first_initial = first[:1]
    if last in raw_tokens and (first in raw_tokens or first_initial in raw_tokens):
        return True
    if last not in normalized_local:
        return False
    return (
        first in normalized_local
        or normalized_local.startswith(f"{first_initial}{last}")
        or normalized_local.startswith(f"{last}{first_initial}")
    )


def _participant_row_matches_name(row: dict[str, Any], *, first: str, last: str) -> bool:
    """Check participant metadata before accepting a fallback email candidate."""
    first_initial = first[:1]
    for field in ("name_normalized", "display_name"):
        tokens = _person_name_tokens(str(row.get(field, "")))
        if last in tokens and (first in tokens or first_initial in tokens):
            return True
    addr = row.get("email_address", "") or ""
    return bool(addr) and _email_candidate_matches_name(addr, first, last)
...
def _query_top_contacts_rows(
    ...
    *,
    ...
    def _run_query(email_value: str, *, operator: str) -> tuple[list[dict], str]:
        if direction == "outbound":
            sql = (
                ...
                f" SUM(d.total_count) AS total"
                f" FROM {ENRON_COMMUNICATION_DYADS_TABLE} d"
                f" WHERE LOWER(d.person_a) {operator} :email_pat"
            ...
            sql = (
                ...
                f" SUM(d.total_count) AS total"
                f" FROM {ENRON_COMMUNICATION_DYADS_TABLE} d"
                f" WHERE LOWER(d.person_b) {operator} :email_pat"
        ...
        return _backend.execute_sql(sql, params={"email_pat": email_value}) or [], sql

    exact_emails: list[str] = []
    for addr in getattr(resolved, "email_addresses", []) or []:
        normalized = str(addr or "").strip().lower()
        if normalized and "@" in normalized and normalized not in exact_emails:
            exact_emails.append(normalized)

    if exact_emails:
        aggregated: dict[str, dict[str, int | str]] = {}
        for email_addr in exact_emails:
            rows, sql = _run_query(email_addr, operator="=")
            last_sql = sql
            for row in rows:
                ...
                existing["sent"] += int(row.get("sent") or 0)
                existing["received"] += int(row.get("received") or 0)
                existing["total"] += int(row.get("total") or 0)
        if aggregated:
            merged = sorted(
                aggregated.values(),
                key=lambda row: (
            ...
            return merged[:safe_limit], last_sql

    for email_pat in resolved.email_patterns:
        normalized = str(email_pat or "").strip().lower()
        if not normalized:
            continue
        results, sql = _run_query(normalized, operator="LIKE")
        last_sql = sql
        if results:
            return results, sql
...
def _build_top_contacts_payload(
    ...
    *,
    ...
    if direction == "both" and contacts:
        all_sent_zero = all(contact.get("sent", 0) == 0 for contact in contacts)
        all_received_zero = all(contact.get("received", 0) == 0 for contact in contacts)
        if all_sent_zero or all_received_zero:
            missing_side = "sent" if all_sent_zero else "received"
            observed_side = "received" if all_sent_zero else "sent"
            payload["data_quality_warning"] = (
                f"All top contacts show {missing_side}=0 while {observed_side} counts are present. "
                "This likely reflects incomplete directional coverage for this entity rather than "
...
def _entity_mention_patterns(resolved: "ResolvedEntity") -> list[str]:
    patterns = list(getattr(resolved, "entity_id_patterns", []) or [])
    entity_id = str(getattr(resolved, "entity_id", "") or "").strip().lower()
    if entity_id:
        exact_pattern = f"%{entity_id}%"
        if exact_pattern not in patterns:
            patterns.append(exact_pattern)
    if CORPUS == "enron" and entity_id:
        try:
            rows = _backend.execute_sql(
                f"SELECT canonical_id FROM {CATALOG}.{ENRON_SCHEMA}.entity_aliases"
                f" WHERE LOWER(alias_id) = :entity_id"
                f" LIMIT 5",
            ...
            for row in rows:
                ...
                if not canonical_id:
                    continue
                canonical_pattern = f"%{canonical_id}%"
                if canonical_pattern not in patterns:
                    patterns.append(canonical_pattern)
        except Exception:
            pass
...
def _query_topic_distribution_rows(
    ...
    *,
    ...
    if entity_name:
        ...
        pattern_values = _entity_mention_patterns(resolved)
        if not pattern_values:
            pattern_values = [f"%{'_'.join(resolved.canonical_name.lower().split())}%"]
        params = {f"pattern_{idx}": pattern for idx, pattern in enumerate(pattern_values)}
        where_clause = "WHERE " + " OR ".join(
            f"LOWER(em.entity_id) LIKE :pattern_{idx}"
            for idx in range(len(pattern_values))
        )
    else:
        where_clause = ""
        params = {}
    ...
    if BACKEND_TYPE == "databricks":
        attempts.append(
            (
                f"WITH parsed AS ("
                f"  SELECT t.thread_id, t.subject,"
                f"   EXPLODE(FROM_JSON(CAST(t.key_topics AS STRING), 'ARRAY<STRING>')) AS topic"
                f"  {join_clause} {where_clause}"
                f")"
                f" SELECT topic, COUNT(DISTINCT thread_id) AS thread_count,"
                f"   FIRST(subject) AS sample_subject"
                f" FROM parsed GROUP BY topic ORDER BY thread_count DESC LIMIT {safe_limit}"
            )
        )
        attempts.extend([
            (
                ...
                f" {where_clause}"
                f" GROUP BY topic ORDER BY thread_count DESC LIMIT {safe_limit}"
            ),
            (
                f"WITH exploded AS ("
                ...
                f" SELECT topic, COUNT(DISTINCT thread_id) AS thread_count,"
                f"   FIRST(subject) AS sample_subject"
                f" FROM exploded GROUP BY topic ORDER BY thread_count DESC LIMIT {safe_limit}"
            ),
        ])
    else:
        attempts.append(
    ...
    for sql in attempts:
        ...
        try:
            ...
            if rows:
                return rows, sql, resolved_entities, resolved_name
        except Exception as exc:
            last_exc = exc
    if last_exc:
        raise last_exc
    return [], last_sql, resolved_entities, resolved_name
...
def _query_browse_topics_rows(
    ...
    *,
    ...
    if entity_name:
        ...
        sql = ""
        try:
            topic_rows, sql, resolved_entities, resolved_name = _query_topic_distribution_rows(
                entity_name,
                limit=20,
                resolved=resolved,
            )
...
def get_topic_distribution(entity_name: str = "", limit: int = 20) -> str:
    ...
    def _run_topic_query(where_clause: str, params: dict) -> list:
        """Try Databricks-native topic SQL or an UNNEST fallback."""
        join_clause = (
            f" FROM {mentions_table} em"
            f" JOIN {threads_table} t ON em.thread_id = t.thread_id"
        ...
        if BACKEND_TYPE == "databricks":
            ...
            for sql in attempts:
                try:
                    return _backend.execute_sql(sql, params=params)
                except Exception as exc:
                    last_exc = exc
            if last_exc:
                raise last_exc
...
def _extract_topic_rows_from_tool_entries(
    tool_entries: list[tuple[str, str]],
    *,
    limit: int = 10,
) -> list[dict]:
    ...
    def _ingest(rows: list[dict]) -> None:
        for row in rows:
            if not isinstance(row, dict):
                continue
            label = str(row.get("topic") or row.get("topic_label") or "").strip()
            if not label:
                continue
            count = int(row.get("thread_count") or 0)
            ...
            if count > int(existing.get("thread_count") or 0):
                existing["thread_count"] = count
            if sample_subject and not existing.get("sample_subject"):
                existing["sample_subject"] = sample_subject

    for call, result in tool_entries:
        tool = _tool_name_from_call(call)
        if tool not in ("get_topic_distribution", "browse_topics", "get_entity_context"):
            continue
        try:
            data = json.loads(result)
        except (json.JSONDecodeError, TypeError, ValueError):
        ...
        if tool == "get_entity_context":
            topic_bundle = data.get("topics")
            if isinstance(topic_bundle, dict):
                _ingest(topic_bundle.get("topics", []))
            continue

        _ingest(data.get("topics", []))
...
def _build_evidence_predigest_block(
    ...
    *,
    ...
    topic_focus = any(
        ...
        for hint in ("topic", "topics", "subject", "subjects", "theme", "themes")
    )
    ...
    topic_rows = _extract_topic_rows_from_tool_entries(tool_entries)
    if topic_focus and topic_rows:
        topic_lines = [
            "\n\n## CURATED TOPIC DISTRIBUTION",
            "The following ranked topics come from structured topic tools.",
            "For topic questions, prefer these labels over incidental phrases from individual email snippets.\n",
        ]
        for row in topic_rows:
            line = f"- **{row.get('topic', '')}**"
            if row.get("thread_count"):
                line += f" ({row['thread_count']} threads)"
            if row.get("sample_subject"):
                line += f" — sample subject: {row['sample_subject']}"
...
def _parse_entity_memory_context(entity_memory_context: str) -> tuple[list[str], str]:
    recent_names: list[str] = []
    tool_context = ""
    if not entity_memory_context:
        return recent_names, tool_context

    for raw_line in entity_memory_context.strip().splitlines():
        line = raw_line.strip()
        if line.startswith("Recent entities from prior conversation:"):
            for part in line.replace("Recent entities from prior conversation:", "").split(","):
                name = part.strip()
                if name and len(name) > 1:
                    recent_names.append(name)
        elif line.startswith("Recent tool context:"):
            tool_context = line.replace("Recent tool context:", "").strip()
    return recent_names, tool_context


def _recent_user_entities_from_history(
    ...
    *,
    ...
    for msg in reversed(conversation_history[-6:]):
        ...
        if not isinstance(content, str) or not content.strip():
            continue
        for name in _heuristic_entity_names(content):
...
def _resolve_coreference(question: str, conversation_history: list[dict], entity_memory_context: str) -> str:
    ...
    demonstrative = re.search(
        r"\b(those|these|the above|the previous|said)\s+"
        r"(emails?|messages?|threads?|conversations?|contacts?|people|topics?)\b",
        question,
        flags=re.IGNORECASE,
    )
    has_pronoun = any(w in question.lower().split() for w in _PRONOUNS)
    if not has_pronoun and not demonstrative:
        return question

    # Long questions likely name their own subjects — skip rewriting.
    if len(question.split()) > 15 and not demonstrative:
        return question

    recent_names, tool_context = _parse_entity_memory_context(entity_memory_context)
    history_names = _recent_user_entities_from_history(conversation_history)
    if history_names:
        recent_names = history_names + [name for name in recent_names if name not in history_names]
    if not recent_names:
        return question
    ...
    for pronoun in ["them", "their", "they"]:
        resolved = re.sub(rf'\b{pronoun}\b', primary, resolved, flags=re.IGNORECASE)
    if demonstrative:
        noun = demonstrative.group(2).lower()
        if noun.startswith(("email", "message", "thread", "conversation")):
            replacement = f"emails involving {primary}"
        elif noun.startswith(("contact", "people")):
            replacement = f"contacts for {primary}"
        else:
            replacement = f"{primary} {noun}"
        resolved = re.sub(
            r"\b(those|these|the above|the previous|said)\s+"
            r"(emails?|messages?|threads?|conversations?|contacts?|people|topics?)\b",
            replacement,
    ...
    if resolved != question:
        if tool_context:
            log.info("Coreference resolved with tool context %r: %r -> %r", tool_context, question, resolved)
        else:
...
def _plan_from_classification(
    ...
    resolved = _resolve_coreference(question, conversation_history or [], entity_memory_context)
...
def _plan_query(
    ...
    """
    # Apply a lightweight, conversation-aware coreference pass before planning.
    # This preserves the active subject for short follow-ups such as
    # "those emails" even when the prior turn only exists in chat history.
    resolved_question = _resolve_coreference(
        question,
...
class EntityMemory:
    ...
    def extract(self, tool_output: str, tool_name: str = ""):
        ...
        try:
            ...
            return
        self._walk(data)
        summary = self._summarize_tool_context(tool_name, data)
        if summary:
            self.last_tool_context = summary
    ...
    def _summarize_tool_context(self, tool_name: str, data: Any) -> str:
        if not tool_name or not isinstance(data, dict):
            return ""
        if tool_name == "find_top_contacts":
            entity = data.get("entity")
            contacts = data.get("top_contacts") or []
            labels = [
                contact.get("name") or contact.get("email")
                for contact in contacts[:3]
                if isinstance(contact, dict) and (contact.get("name") or contact.get("email"))
            ]
            if entity and labels:
                return f"find_top_contacts for {entity} returned {', '.join(labels)}"
        if tool_name in {"browse_topics", "get_entity_context"}:
            entity = data.get("entity")
            topics = data.get("topics") or []
            if not isinstance(topics, list):
                return ""
            labels = [
                topic.get("topic") or topic.get("topic_label")
                for topic in topics[:3]
                if isinstance(topic, dict) and (topic.get("topic") or topic.get("topic_label"))
            ]
            if entity and labels:
                return f"{tool_name} for {entity} returned topics {', '.join(labels)}"
        return ""
    ...
    def context_for_classifier(self) -> str:
        ...
        """
        if not self.recent and not self.user_mentioned and not self.last_tool_context:
            return ""
        ...
        if not all_names and not self.last_tool_context:
            return ""
        lines = []
        if all_names:
            lines.append(f"Recent entities from prior conversation: {', '.join(all_names)}")
        if self.last_tool_context:
            lines.append(f"Recent tool context: {self.last_tool_context}")
        return "\n" + "\n".join(lines) + "\n"
...
class GraphRAGAgent(ResponsesAgent):
    ...
    def _execute_fast_path_stream(
        ...
        *,
        ...
        for call_key, result_str in tool_results.items():
            if isinstance(result_str, str):
                tool_name, _ = _parse_stream_tool_entry(call_key)
                self.entity_memory.extract(result_str, tool_name=tool_name)
        ...
        if followup_steps:
            for step in followup_steps:
                ...
                if isinstance(result, str):
                    self.entity_memory.extract(result, tool_name=step.tool_name)
                yield ResponsesAgentStreamEvent(
                    type="response.output_item.done",
                    item=create_function_call_output_item(
                        call_id=call_id,
        ...
        if _should_run_targeted_documentary_retry(
            ...
            for call_key, result in retry_tool_results.items():
                if isinstance(result, str):
                    tool_name, _ = _parse_stream_tool_entry(call_key)
                    self.entity_memory.extract(result, tool_name=tool_name)
    ...
    def predict_stream(
        ...
        try:
            ...
            if question and CORPUS == "enron" and TOOL_MAP:
                resolved_for_classification = _resolve_coreference(question, messages, em_context)
                classify_question = (
                    resolved_for_classification + em_context
                    if em_context else resolved_for_classification
                )
                answer_contract = _extract_answer_contract(resolved_for_classification)
                routing_hint = _get_case_based_pattern_hint(resolved_for_classification, answer_contract)
                pre_classification = classify_and_extract(
                    classify_question,
                    raw_question=resolved_for_classification,
                    ...
                    for node_data in event[1].values():
                        for msg in node_data.get("messages", []):
                            if isinstance(msg, AIMessage) and msg.tool_calls:
                                ...
                                self.entity_memory.extract(tool_content, tool_name=getattr(msg, "name", "") or "")
                                yield ResponsesAgentStreamEvent(
                                    type="response.output_item.done",
                                    item=create_function_call_output_item(
